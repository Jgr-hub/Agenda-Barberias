...
export default function ClientBooking() {
  ...
  const fetchBarbershop = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/barbershop/${barbershopId}/public`)
      if (response.ok) {
  ...
  const fetchSlots = useCallback(async (date) => {
    ...
    try {
      const response = await fetch(`${API_URL}/barbershop/${barbershopId}/slots?date=${date}`)
      if (response.ok) {
        const data = await response.json()
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true)
    try {
      const response = await fetch(`${API_URL}/barbershop/${barbershopId}/appointments`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        alert(err.detail || 'No se pudo crear la cita')
      }
    } catch (err) {
    ...
    <div className="booking-page">
      ...
      <header className="booking-header">
        {barbershop.photo ? (
          ...
          <div className="shop-photo-placeholder">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
