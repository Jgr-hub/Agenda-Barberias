...
export default function Admin() {
  ...
  const fetchSlots = useCallback(async () => {
    if (!token) return
    try {
      const response = await fetch(`${API_URL}/slots/all`, {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      if (response.ok) {
        const data = await response.json()
  ...
  const updateAppointmentStatus = async (id, status) => {
    try {
      ...
      if (response.ok) {
        alert(status === 'confirmed' ? 'Cita confirmada' : 'Cita rechazada')
        fetchAppointments()
        fetchSlots()
      }
    } catch (error) {
  ...
  const addSlot = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(error.detail || 'No se pudo agregar')
      }
    } catch (error) {
      alert('Error de conexión')
    } finally {
  ...
  const quickAddSlots = async () => {
    if (!newSlotDate) {
      alert('Ingresa una fecha primero')
      return
    }
    ...
    try {
      const slotsToCreate = defaultTimes.map(time => ({ date: newSlotDate, time }))
      ...
      if (response.ok) {
        ...
        setNewSlotDate('')
        fetchSlots()
        alert(`${created.length} horarios agregados`)
      }
    } catch (error) {
      alert('Error de conexión')
    } finally {
  ...
  const copyLink = () => {
    if (barbershop) {
      const link = `${window.location.origin}/b/${barbershop.id}`
                    ...
                    {slots.map((slot) => (
                      <div key={slot.id} className="slot-card">
                        ...
                        <div className="slot-status">
                          <span className={`availability-badge ${slot.is_available ? 'available' : 'booked'}`}>
                ...
                {editingProfile ? (
                  <div className="profile-actions">
                    ...
                    <button
                      ...
                      disabled={submitting}
                    >
                      {submitting ? <div className="spinner small"></div> : 'Guardar'}
                    </button>
                  </div>
                ) : (
                ...
                <div className="link-section">
                  ...
                  <div className="link-container">
                    <span className="link-text">
                      {window.location.origin}/b/{barbershop?.id}
                    </span>
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={addSlot}>
              ...
              <button type="submit" className="submit-btn" disabled={submitting}>
                {submitting ? <div className="spinner small"></div> : 'Agregar horario'}
              </button>
              <button
                type="button"
                className="quick-add-btn"
                onClick={quickAddSlots}
                disabled={submitting}
              >
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
